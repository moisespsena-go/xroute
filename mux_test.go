...
func TestMuxBasic(t *testing.T) {
	var count uint64
	countermw := func(next *ChainHandler) {
		count++
		next.Next()
	}

	usermw := func(next *ChainHandler) {
		next.Next(context.WithValue(next.request.Context(), ctxKey{"user"}, "peter"))
	}

	exmw := func(next *ChainHandler) {
		next.Next(context.WithValue(next.request.Context(), ctxKey{"ex"}, "a"))
	}

	logbuf := bytes.NewBufferString("")
	logmsg := "logmw test"
	logmw := func(next *ChainHandler) {
...
func TestMuxNestedNotFound(t *testing.T) {
	...
	r.Use(func(next *ChainHandler) {
		next.request = next.request.WithContext(context.WithValue(next.request.Context(), ctxKey{"mw"}, "mw"))
		next.Next()
	})
	...
	r.With(func(next *ChainHandler) {
		next.request = next.request.WithContext(context.WithValue(next.request.Context(), ctxKey{"with"}, "with"))
		next.Next()
	...
	sr1.Group(func(sr1 Router) {
		sr1.Use(func(next *ChainHandler) {
			next.request = next.request.WithContext(context.WithValue(next.request.Context(), ctxKey{"mw2"}, "mw2"))
			next.Next()
		})
...
func TestMuxWith(t *testing.T) {
	...
	mw1 := func(next *ChainHandler) {
		cmwInit1++
		cmwHandler1++
		next.request = next.request.WithContext(context.WithValue(next.request.Context(), ctxKey{"inline1"}, "yes"))
		next.Next()
	}
	mw2 := func(next *ChainHandler) {
		cmwInit2++
		cmwHandler2++
		next.request = next.request.WithContext(context.WithValue(next.request.Context(), ctxKey{"inline2"}, "yes"))
		next.Next()
	}
...
func TestMuxMiddlewareStack(t *testing.T) {
	...
	var ctxmwHandler uint64
	ctxmw := func(next *ChainHandler) {
		ctxmwHandler++
		ctx := next.request.Context()
		ctx = context.WithValue(ctx, ctxKey{"count.ctxmwHandler"}, ctxmwHandler)
		next.Next(next.request.WithContext(ctx))
	}

	var inCtxmwInit, inCtxmwHandler uint64
	inCtxmw := func(next *ChainHandler) {
		inCtxmwInit++
		inCtxmwHandler++
		next.Next()
	}
	...
	r.Use(ctxmw)
	r.Use(func(next *ChainHandler) {
		if next.request.URL.Path == "/ping" {
			next.Writer.Write([]byte("pong"))
			return
		}
		next.Next()
	})
	...
	r.With(inCtxmw).Get("/", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(fmt.Sprintf("reqs:%d ctxValue:%d", handlerCount, ctxmwHandlerCount)))
	})
...
func bigMux() Router {
	...
	r = NewRouter()
	r.Use(func(next *ChainHandler) {
		ctx := context.WithValue(next.request.Context(), ctxKey{"requestID"}, "1")
		next.Next(next.request.WithContext(ctx))
	})
	r.Use(func(next *ChainHandler) {
		next.Next()
	...
	r.Group(func(r Router) {
		r.Use(func(next *ChainHandler) {
			next.Next(context.WithValue(next.request.Context(), ctxKey{"session.user"}, "anonymous"))
		})
	...
	r.Group(func(r Router) {
		...
		r.Route("/hubs", func(r Router) {
			...
			r.Route("/{hubID}", func(r Router) {
				...
				r.Mount("/webhooks", Chain(NewMiddleware(func(next *ChainHandler) {
					next.Next(context.WithValue(next.request.Context(), ctxKey{"hook"}, true))
				})).Handler(sr3))
...
func TestNestedGroups(t *testing.T) {
	...
	mwIncreaseCounter := func(next *ChainHandler) {
		ctx := next.request.Context()
...
func TestEscapedURLParams(t *testing.T) {
	...
	m.Get("/api/{identifier}/{region}/{size}/{rotation}/*", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
		rctx := RouteContextFromContext(r.Context())
		if rctx == nil {
			t.Error("no context")
			return
		}
