...
func (n *node) GetRoute(method MethodType, pattern string) *EndpointHandler {
	...
	for {
		...
		if len(search) == 0 {
			return n.endpoints[method].handler
		}
...
func (n *node) FindRoute(rctx *RouteContext, method MethodType, path string, header ...http.Header) (*node, endpoints, ContextHandler) {
	...
	for i, key := range rctx.routeParams.Keys {
		rctx.URLParams.Add(key, rctx.routeParams.Values[i])
	}
...
func walk(r Routes, walkFn WalkFunc, parentRoute string, parentMw ...*Middleware) error {
	for _, route := range r.Routes() {
		mws := make([]*Middleware, len(parentMw))
		copy(mws, parentMw)
